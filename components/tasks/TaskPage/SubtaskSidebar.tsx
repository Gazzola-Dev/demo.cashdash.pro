...
type Subtask = Partial<Tables<"subtasks">>;
...
export function SubtaskSidebar() {
  ...
  const handleEdit = (subtask: Subtask) => {
    if (subtask.id) setEditingId(subtask.id);
    if (subtask.title) setEditedTitle(subtask.title);
  };
  ...
  const handleToggleComplete = (subtask: Subtask) => {
    if (subtask.id) setIsLoading(subtask.id);
  };
