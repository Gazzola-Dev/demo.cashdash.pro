...
DROP FUNCTION IF EXISTS public.list_projects(text, text, text, text);

-- Create updated function with user access control
...
BEGIN
  ...
  FROM projects p
  -- Join to project_members first to filter for user access
  INNER JOIN project_members user_pm ON p.id = user_pm.project_id AND user_pm.user_id = auth.uid()
  -- Then get all other project data
...
GRANT EXECUTE ON FUNCTION public.list_projects(TEXT, TEXT, TEXT, TEXT) TO authenticated;