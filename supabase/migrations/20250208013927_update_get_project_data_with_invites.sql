...
-- Create function to get project data with all relationships and invited users
...
BEGIN
  ...
  project_invites_clean AS (
    -- Subquery to get pending invitations with inviter profile data
    SELECT DISTINCT ON (pi.email)
      jsonb_build_object(
        'id', pi.id,
    ...
    WHERE pi.project_id = found_project.id
    AND pi.status = 'pending'
    ORDER BY pi.email, pi.created_at DESC
  )
  SELECT 
    jsonb_build_object(
      'id', p.id,
      ...
      'project_invitations', COALESCE(
        (SELECT jsonb_agg(invite_data)
         FROM project_invites_clean),
        '[]'::jsonb
      ),
      'tasks', COALESCE(
        jsonb_agg(
