...
BEGIN
  ...
  IF task_updates ? 'task_schedule' THEN
    IF jsonb_array_length(task_updates->'task_schedule') > 0 THEN
      ...
      SELECT 
        updated_task.id,
        COALESCE(schedule_data.due_date, task_schedule.due_date),
        COALESCE(schedule_data.start_date, task_schedule.start_date),
        COALESCE(schedule_data.estimated_hours, task_schedule.estimated_hours),
        COALESCE(schedule_data.actual_hours, task_schedule.actual_hours)
      FROM schedule_data
      LEFT JOIN task_schedule ON task_schedule.task_id = updated_task.id
